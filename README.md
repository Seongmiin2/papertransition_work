...
- `semantic`: `fidelity` 배경 위에 검출된 표 셀과 그림 영역을 별도 편집 개체로 추가합니다. 데스크톱 앱의 현재 기본값입니다.
...
한 폴더의 PDF를 연속 변환할 때는 아래 명령을 사용합니다. OCR 모델을 한 번만 로드하고, 실패 파일 때문에 전체 작업이 멈추지 않으며, 같은 설정으로 재실행하면 완료 파일은 건너뜁니다.

```powershell
python -m scan2hwpx batch-convert "C:\시험지" --out "C:\변환결과" --mode fast --dpi 240
```
